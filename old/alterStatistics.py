...
class alterStatistics:
    ...
    def alter_stats_by_field_combo(
        ...
        ac_boost=0,
        apply_base_armor_class=0,
        lock_block_boolean=None,
        map_applied_boolean=None,
    ):
        for block in blocks:
            if block.lock_block:
                continue
    ...
    def passes_match_phrases(
        ...
        if lock_block_boolean is not None:
            if block.lock_block != lock_block_boolean:
                return False

        if map_applied_boolean is not None:
            if block.map_applied != map_applied_boolean:
                return False
    ...
    def print_dry_run_summary(diffs, blocks):
        ...
        for changes in diffs.values():
            ...
            if "PassivesToAdd" in changes:
                old, new = changes["PassivesToAdd"]
                passives_added += max(0, len(new or []) - len(old or []))
            if "SpellsToAdd" in changes:
                old, new = changes["SpellsToAdd"]
                spells_added += max(0, len(new or []) - len(old or []))

        print("---- Dry Run Summary (alterStatistics) ----")
        print(f"Blocks loaded: {len(blocks)}")
        print(f"Blocks skipped (LockBlock): {locked_count}")
        print(f"Blocks updated: {len(diffs)}")
        print(f"Passives added (net): {passives_added}")
        print(f"Spells added (net): {spells_added}")
        if field_counts:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Apply bulk passive/spell/health/AC changes to guid_mapper_master.json."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # lock_block_boolean/map_applied_boolean: same None/True/False convention.
    # Blocks with LockBlock=True are always skipped outright, regardless of these filters.
    ...
    if args.dry_run:
        alterStatistics.print_dry_run_summary(diffs, blocks)
